...
// 알림 체크 로직
async function checkNotifications(skipDateCheck: boolean = false): Promise<string[]> {
    ...
    for (const vehicle of vehicles) {
        ...
        for (const setting of settings.consumableSettings || []) {
            ...
            if (distanceAlert || timeAlert) {
                ...
                if (distanceAlert) msg += ` (잔여 ${remainingKm}km)`;
                if (timeAlert) msg += ` (만료 ${remainingDays}일 전)`;
                alertLines.push(msg);
            }
        }
    }
 ...
 */
export const runNotificationCheck = onRequest({
    region: 'us-central1',
    cors: true
}, async (req, res) => {
