  ...
  return (
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="p-8 space-y-6 text-sm text-gray-800 leading-relaxed font-sans">
          <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4">
            ...
            <p className="text-gray-600">구글 시트 연동을 위한 Apps Script 설정 방법입니다. <span className="font-bold text-blue-600">(v5.4.1)</span></p>
          </div>
    ...
    try {
        ...
        if (data.action === 'delete') {
            ...
            // Read all Item IDs (Column 23 / Index 22)
            // Range: Row 2, Col 23, NumRows, 1
            // Read all Item IDs (Column 24 / Index 23)
            // Range: Row 2, Col 24, NumRows, 1
            const idRange = sheet.getRange(2, 24, lastRow - 1, 1);
            const idValues = idRange.getValues(); // [[id1], [id2], ...]
            
            let found = false;
            // Loop backwards to handle potential multiple deletes safe (though ID should be unique)
            for (let i = idValues.length - 1; i >= 0; i--) {
                if (idValues[i][0] === data.id) {
                    sheet.deleteRow(i + 2); // Row index is 1-based, and data starts at row 2
                    found = true;
                    // break; // If unique, break. If duplicates possible, continue? Let's assume unique.
                    break;
                }
            }
        ...
        if (data.action === 'update') {
             if (!data.id) return ContentService.createTextOutput(JSON.stringify({ result: 'error', message: 'No ID provided for update' })).setMimeType(ContentService.MimeType.JSON);
             
             const lastRow = sheet.getLastRow();
             if (lastRow < 2) return ContentService.createTextOutput(JSON.stringify({ result: 'error', message: 'Sheet empty, cannot update' })).setMimeType(ContentService.MimeType.JSON);

             // Read all Item IDs (Column 24 / Index 23) - Shifted for endDate
             // Range: Row 2, Col 24, NumRows, 1
             const idRange = sheet.getRange(2, 24, lastRow - 1, 1);
             ...
             for (let i = 0; i < idValues.length; i++) {
                 if (idValues[i][0] === data.id) {
                     foundIndex = i + 2; // Make it 1-based and skip header
                     break;
                 }
             }

             if (foundIndex === -1) {
                 return ContentService.createTextOutput(JSON.stringify({ result: 'error', message: 'Log ID not found for update' })).setMimeType(ContentService.MimeType.JSON);
             }

             // Prepare Data (Similar to Append)
             ...
             const row = [
                 ...
                 data.endDate || data.startDate || data.date,   // 2. 종료일 (NEW)
                 ...
                 data.amount || '',                             
                 data.pricePerLiter || '',                      
                 data.destination || data.station || data.shop || '',               // 15. 장소(목적지/주유소/정비소)
                 data.item || '',                               
                 data.cost || '',                               
                 data.paymentMethod || '',                      
        ...
        if (!note) {
            ...
            if (data.stopovers && data.stopovers.length > 0) note += \`경유: \${data.stopovers.length}곳 \`;
                ...
                // --- NEW: Save Image to Drive ---
                let driveImageUrl = '';
                if (data.imageUrl) {
                  console.log("Image URL received: " + data.imageUrl); // Log for debugging
                driveImageUrl = saveImageToDrive(data.imageUrl);
                // Error Handling: If it starts with 'Error:', preserve it as text to show in sheet
                console.log("Drive Image Result: " + driveImageUrl);
        }
                // --------------------------------
                ...
                (data.type === 'fueling' && driveImageUrl) ? (driveImageUrl.startsWith('Error') ? driveImageUrl : \`=IMAGE("\${driveImageUrl}")\`) : '',      // 21. 주유이미지 (Drive Link)
            (data.type === 'maintenance' && driveImageUrl) ? (driveImageUrl.startsWith('Error') ? driveImageUrl : \`=IMAGE("\${driveImageUrl}")\`) : '',  // 22. 정비이미지 (Drive Link)
                new Date().toLocaleString(),                   // 23. 등록일시
                data.id || ''                                  // 24. Log ID (For Sync)
                ];
                ...
                return ContentService.createTextOutput(JSON.stringify({result: 'success' })).setMimeType(ContentService.MimeType.JSON);
                  ...
                  console.error("Main Error: " + error.toString()); // Log main errors
