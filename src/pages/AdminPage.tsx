...
import { ArrowLeft, Car, FileText, Settings, X, Download, Edit2, Trash2, Plus, FileDown } from 'lucide-react';
                    ...
                    <div className="bg-white rounded-xl shadow p-4">
                        ...
                        <div className="space-y-2 max-h-[60vh] overflow-y-auto">
                            {filteredLogs.slice(0, 100).map((log, idx) => {
                                ...
                                return (
                                    <div key={log.id || idx} className="border bg-white p-3 rounded-lg shadow-sm hover:shadow-md transition">
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex items-center space-x-2">
                                                <span className={`px-2 py-0.5 rounded text-xs font-bold ${log.type === 'driving' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>{log.typeKr}</span>
                                            ...
                                            <div className="flex space-x-1">
                                                ...
                                                <button
                                                    ...
                                                    className={`p-1 px-2 rounded text-xs ${processingLogId === log.id ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-red-50 text-red-500 hover:bg-red-100'}`}
                                                >
                                                    {processingLogId === log.id ? '삭제 중...' : '삭제'}
                                                </button>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 mb-2">
                                            <div><span className="font-bold text-gray-400">차량:</span> {vehicleDisplay}</div>
                                            <div><span className="font-bold text-gray-400">사용자:</span> {userDisplay}</div>
                                        </div>
                                        <div className="mt-2 text-sm text-gray-700 bg-gray-50 p-2 rounded">
                                            {log.type === 'driving' && (
                                                <div className="space-y-1">
                                                    <div><span className="font-bold">목적:</span> {log.purpose} {log.detailPurpose ? `(${log.detailPurpose})` : ''}</div>
                                                    <div><span className="font-bold">시간:</span> {log.startTime ? new Date(log.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '-'} ~ {log.endTime ? new Date(log.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '-'}</div>
                                                    <div><span className="font-bold">거리:</span> {log.totalDistance}km ({log.startMileage} ~ {log.endMileage})</div>
                                                    <div><span className="font-bold">목적지:</span> {log.destination}</div>
                                                    {log.stopovers && log.stopovers.length > 0 && (
                                                        <div><span className="font-bold text-gray-500">경유지:</span> {log.stopovers.map((s: any) => s.location).join(', ')}</div>
                                                    )}
                                            ...
                                            {log.type === 'fueling' && (
                                                <div className="space-y-1">
                                                    <div className="flex justify-between">
                                                        <span><span className="font-bold">주유소:</span> {log.station}</span>
                                                        <span className="font-bold text-blue-600">{(Number(log.cost) || 0).toLocaleString()}원</span>
                                                    </div>
                                                    <div><span className="font-bold">주유량:</span> {log.amount}L {log.price ? `(${Number(log.price).toLocaleString()}원/L)` : ''}</div>
                                                    <div><span className="font-bold">결제:</span> {log.fundingSource || '-'}</div>
                                                </div>
                                            )}
                                            {log.type === 'maintenance' && (
                                                <div className="space-y-1">
                                                    <div className="flex justify-between">
                                                        <span><span className="font-bold">항목:</span> {log.item}</span>
                                                        <span className="font-bold text-red-600">{(Number(log.cost) || 0).toLocaleString()}원</span>
                                                    </div>
