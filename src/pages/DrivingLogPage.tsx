...
import { ArrowLeft, Plus, Minus, Camera, X, FileText, ArrowUpNarrowWide, ArrowDownNarrowWide } from 'lucide-react';
import { sendToGoogleSheet } from '../utils/googleSheets';
import { sendSlackNotification, sendModificationNotification } from '../utils/slackUtils';
...
const DrivingLogPage: React.FC = () => {
    ...
    const sortedLogs = React.useMemo(() => {
        return [...logs].sort((a, b) => {
            ...
            // Priority 2: Start Time (if available) - Assuming format HH:mm or ISO string
            // Convert time string to comparable value
            const getTimeValue = (item: any) => {
                if (item.startTime) {
                    // Check if it's full ISO string or just HH:mm
                    if (item.startTime.includes('T')) return new Date(item.startTime).getTime();
                    // If HH:mm, we can just compare strings or use dummy date
                    return parseInt(item.startTime.replace(':', ''));
                }
            ...
            // Priority 3: Creation Timestamp (id as proxy if timestamp unavailable, but timestamp preferred)
            const getTimestamp = (item: any) => item.timestamp?.seconds || 0;
            ...
            // Fallback: created at (if available) or random stable sort (id)
            return 0;
        });
    }, [logs, sortOrder]);
    ...
    const [paymentMethods, setPaymentMethods] = useState<string[]>([]); // New State
    ...
    const handleFuelingSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const targetSheetUrl = sheetConfig?.url || localStorage.getItem('GOOGLE_SHEET_WEBHOOK_URL');
            if (targetSheetUrl) {
                console.log(`Sending fueling log to Sheet...`, sheetPayload);
                await sendToGoogleSheet(sheetPayload, targetSheetUrl);
            } else {
                console.warn("Sheet URL missing, skipping sync");
            }
    ...
    const handleMaintenanceSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const targetSheetUrl = sheetConfig?.url || localStorage.getItem('GOOGLE_SHEET_WEBHOOK_URL');
            if (targetSheetUrl) {
                console.log(`Sending maintenance log to Sheet...`, sheetPayload);
                await sendToGoogleSheet(sheetPayload, targetSheetUrl);
            } else {
                console.warn("Sheet URL missing, skipping sync");
            }
    ...
    const handleReasonSubmit = async (reason: string) => {
        ...
        try {
            ...
            await addDoc(collection(db, 'modificationRequests'), requestData);

            // Send Notification (Integrated Template)
            // Use systemSettings if available, or fallback to object with slackWebhook
            const effectiveSettings = systemSettings || { slackWebhook };
            await sendModificationNotification(effectiveSettings, requestData);
                    ...
                    activeTab === 'history' && (
                        <div className="space-y-4 max-w-md mx-auto">
                            ...
                            <div className="flex justify-between items-center px-1 mb-2 mt-4">
                                <div className="flex items-center gap-2">
                                    <span className="text-xs font-bold text-gray-700">총 {sortedLogs.length}건</span>
                                </div>
                                <button
                                    onClick={() => setSortOrder(prev => prev === 'desc' ? 'asc' : 'desc')}
                                    className="flex items-center gap-1 bg-white border border-gray-200 px-2 py-1 rounded-lg text-xs font-medium text-gray-600 hover:bg-gray-50 transition-colors"
                                >
                                    {sortOrder === 'desc' ? (
                                        <>
                                            <ArrowDownNarrowWide size={14} className="text-blue-600" />
                                            <span>최신순</span>
                                        </>
                                    ) : (
                                        <>
                                            <ArrowUpNarrowWide size={14} className="text-orange-600" />
                                            <span>과거순</span>
                                        </>
                                    )}
                                </button>
