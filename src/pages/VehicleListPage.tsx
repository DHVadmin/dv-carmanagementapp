...
const VehicleListPage: React.FC = () => {
    ...
    const seedData = async () => {
        ...
        try {
            ...
            await fetchVehicles();
            alert('?åÏä§???∞Ïù¥?∞Í? ?ùÏÑ±?òÏóà?µÎãà??');
        } catch (e) {
            console.error('Error seeding data:', e);
            alert('?∞Ïù¥???ùÏÑ± ?§Ìå® (Í∂åÌïú Î¨∏Ï†ú?????àÏäµ?àÎã§): ' + e);
        } finally {
            setSeeding(false);
        }
    };
    ...
    const getRoleDisplayName = () => {
        if (!userRole) return '?¨Ïö©??;
        if (userRole === 'admin') return 'Í¥ÄÎ¶¨Ïûê';
        if (userRole === 'approver') return 'Í≤∞Ïû¨Í∂åÏûê';
        return '?¨Ïö©??;
    };
        ...
        <div className="min-h-screen bg-gray-50 pb-20">
            ...
            <header className="bg-white shadow-sm sticky top-0 z-10">
                <div className="max-w-md mx-auto px-4 py-4">
                    <div className="flex justify-between items-center mb-3">
                        <h1 className="text-lg font-bold text-gray-800">?ôÌñâÎπåÎ¶¨ÏßÄ Ï∞®ÎüâÍ¥ÄÎ¶?<span className="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded-full align-middle">v5.5.3</span></h1>
                        <div className="flex items-center space-x-2">
                            ...
                            >
                                ...
                                <span className="whitespace-nowrap">Í∞Ä?¥Îìú</span>
                            </button>
                            {isAdmin && (
                                <button
                                ...
                                >
                                    ...
                                    <span className="whitespace-nowrap">Í¥ÄÎ¶¨Ïûê</span>
                                </button>
                            )}
                            <button
                            ...
                            >
                                <BookOpen size={16} className="mb-0.5" />
                                <span className="whitespace-nowrap">?òÏùò ?îÏ≤≠</span>
                            </button>
                    ...
                    <div className="bg-blue-50 rounded-lg p-3 flex items-start space-x-3">
                        ...
                        <div>
                            <div className="flex items-center space-x-2">
                                <span className="font-bold text-gray-900">{user?.displayName || '?¨Ïö©??}</span>
            ...
            <main className="max-w-md mx-auto px-4 py-6">
                <div className="flex justify-between items-end mb-4">
                    <h2 className="text-xl font-bold text-gray-900">Ï∞®Îüâ ?†ÌÉù</h2>
                    {vehicles.length === 0 && (
                        <button
                            onClick={seedData}
                            disabled={seeding}
                ...
                {vehicles.length === 0 ? (
                    <div className="text-center py-10 text-gray-500 bg-white rounded-xl border border-dashed">
                        <p className="mb-2">?±Î°ù??Ï∞®Îüâ???ÜÏäµ?àÎã§.</p>
                        <button
                            onClick={seedData}
                            disabled={seeding}
                            className="px-4 py-2 bg-blue-50 text-blue-600 rounded-lg text-sm font-medium hover:bg-blue-100 transition-colors"
                        >
                            {seeding ? '?∞Ïù¥???ùÏÑ± Ï§?..' : '?åÏä§???∞Ïù¥???ùÏÑ±?òÍ∏∞'}
                        </button>
                    </div>
                ) : (
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col shadow-2xl">
                        <div className="p-4 border-b flex justify-between items-center bg-gray-50">
                            <h3 className="text-xl font-bold text-gray-800">?ì± ???¨Ïö©??Í∞Ä?¥Îìú</h3>
